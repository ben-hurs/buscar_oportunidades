# busca_por_link.py - Versão Otimizada e Flexível
import asyncio
import os
import json
import logging
...
# Parâmetros de performance padrão (podem ser sobrescritos)
...
async def salvar_links_json(nome_empresa, dados):
    caminho = os.path.join(DATA_DIR, f"links_{nome_empresa.lower().replace(' ', '_')}.json")
    with open(caminho, "w", encoding="utf-8") as f:
        json.dump(dados, f, ensure_ascii=False, indent=2)

async def carregar_links_json(nome_empresa):
    caminho = os.path.join(DATA_DIR, f"links_{nome_empresa.lower().replace(' ', '_')}.json")
...
async def buscar_processos_por_nome(nome_empresa, max_browsers=DEFAULT_MAX_BROWSERS):
    ...
    cached_data = await carregar_links_json(nome_empresa)
    if cached_data:
        logging.info(f"Retornando dados em cache para {nome_empresa}")
        return cached_data
    ...
    async with async_playwright() as p:
        ...
        if not todos_links:
            ...
            await salvar_links_json(nome_empresa, todos_links)
